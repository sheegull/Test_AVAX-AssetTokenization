...
import { avaxToWei, weiToAvax, blockTimeStampToDate } from "../../utils/formatter";
...
export default function ListNftForm() {
    ...
    const getAllNftDetails = useCallback(async () => {
        ...
        for (let index = 0; index < farmers.length; index++) {
            const available = await assetTokenization.availableContract(farmers[index]);
            if (available) {
                const details = await assetTokenization.getNftContractDetails(farmers[index]);
                allDetails.push({
